...
public class ChromeDownloader extends BasePage implements ElementSearchers, ShadowRootManipulation {
	 ...
	 * 
	 * Also we can check process of loading through the "Pause" button is it
	 * displayed we are downloading the file
	 * 
	 * In case is "Show in Folder" not exist and "Pause" is not displayed we have
	 * another buttons "Keep/Discard" or "Resume" in this case we have a problem
	 * with loading
	 * 
	...
	private boolean check(WebElement expandedShadow) {
		WebElement pauseButton = elementById("description", expandedShadow);
		while (pauseButton.isDisplayed()) {
			sleep(1000);
		}
		try {
